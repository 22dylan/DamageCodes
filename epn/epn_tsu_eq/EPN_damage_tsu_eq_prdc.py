...
class EPN_damage_tsu_eq():
	...
	def run(self):
		...
		# 		for self.r in range(len(self.RT)): 		#loop through return intervals
		# 			if (self.retrofit == 2) or (self.retrofit == 3):				#if the SS is elevatred by 5 ft
		# 				self.IM_SS_d[self.r] = self.IM_SS_d[self.r] - 5/3.2808
		# 			self.define_flowspeeds_SS()
		# 			self.define_SS_frag_tsu_variables()
		# 			self.wtp_eval()

		self.write_tax_lot_conn()
		# self.reorg_gis_data()
		# self.write_pole_failure()
	...
	def initialize_variables(self):
		...
		self.outfilename = 'epn_damage_tsu_eq'
	...
	def preallocate_space(self):
		...
		self.rep_cost_poles_tsu = np.zeros(self.fast_vals*self.retrofit_vals*RT_size*self.n_sims).reshape(self.fast_vals, self.retrofit_vals, RT_size, self.n_sims)
		self.rep_cost_poles_eq = np.zeros(self.fast_vals*self.retrofit_vals*RT_size*self.n_sims).reshape(self.fast_vals, self.retrofit_vals, RT_size, self.n_sims)

		self.rep_time_SS_gen_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*5).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, 5)	# note: drs, look into if "5" can be hardcoded. 
		self.rep_time_SS_gen_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*5).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, 5)	# note: drs, look into if "5" can be hardcoded. 
		self.DS_SS_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.DS_SS_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.rep_time_SS_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.rep_time_SS_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.rep_cost_SS_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.rep_cost_SS_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.func_SS_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
	...
	def define_pole_frag_variables(self):
		...
		self.pole_frag_covm_eq = self.pole_frag_beta_eq[:,None]*self.pole_frag_beta_eq 	# roadway fragility covariance matrix
	...
	def conn_analysis1(self):
		...
		np.random.seed(self.i)
		Capacity_poles_eq, rep_time_poles_all_eq = DCH.capacity_reptime_gen(self.pole_frag_med_eq, self.pole_frag_covm_eq, self.pole_rep_time_log_med_eq, self.pole_rep_time_covm_eq, self.n_nodes)
		...
		_, DS_poles_tsu = DCH.DS_eval(self.IM_pole_d, Capacity_poles_tsu, 3)
		...
		self.down_poles_l_eq, DS_poles_eq = DCH.DS_eval(self.IM_pole_pga, Capacity_poles_eq, 3)
		DS_poles_eq[self.pole_y_n==0] = 1
		self.down_poles_l =  np.logical_or(self.down_poles_l_eq, self.down_poles_l_tsu) # poles awith damage greater than moderate damage are assumed to be not functional
		self.down_poles = np.where(self.down_poles_l)[0]

		# self.down_poles_l = self.down_poles_l_tsu[:]
		self.rep_cost_poles_tsu[self.fast, self.retrofit, self.r, self.i] = np.sum(self.damage_ratio_poles_tsu[DS_poles_tsu.astype(int)-1] * self.pole_cost)
		self.rep_time_poles_tsu = np.array([rep_time_poles_all_tsu[int(i), int(obj)-1] for i, obj in enumerate(DS_poles_tsu)]) #* self.down_poles_l

		self.rep_cost_poles_eq[self.fast, self.retrofit, self.r, self.i] = np.sum(self.damage_ratio_poles_eq[DS_poles_eq.astype(int)-1] * self.pole_cost)
		self.rep_time_poles_eq = np.array([rep_time_poles_all_eq[int(i), int(obj)-1] for i, obj in enumerate(DS_poles_eq)]) #* self.down_poles_l

		self.rep_time_poles = np.ceil((np.amax((self.rep_time_poles_eq, self.rep_time_poles_tsu), axis=0))) * self.down_poles_l
		self.rep_cost_poles[self.fast, self.retrofit, self.r, self.i] = np.amax((self.rep_cost_poles_tsu[self.fast, self.retrofit, self.r, self.i], self.rep_cost_poles_eq[self.fast, self.retrofit, self.r, self.i]))
	...
	def define_SS_frag_variables(self):
		...
		elif (self.retrofit == 3) or (self.retrofit == 1): # anchored
	...
	def define_SS_frag_tsu_variables(self):
	...
	def define_SS_rep_variables(self):
		if self.fast == 0:
			...
			self.SS_rep_time_std_eq = np.array([0.5, 1.5, 3.5, 15]) # std for repair time	
		else:
			...
			self.SS_rep_time_std_eq = np.array([0.5, 1.5, 3.5, 15])*0.5 # std for repair time	
		...
		self.rep_time_SS_gen_tsu[self.fast, self.retrofit] = np.column_stack((np.zeros(self.n_sims), np.exp(np.random.multivariate_normal(self.SS_rep_time_log_med_tsu, self.SS_rep_time_covm_tsu, self.n_sims))))
		self.rep_time_SS_gen_eq[self.fast, self.retrofit] = np.column_stack((np.zeros(self.n_sims), np.exp(np.random.multivariate_normal(self.SS_rep_time_log_med_eq, self.SS_rep_time_covm_eq, self.n_sims))))
	...
	def wtp_eval(self):
		...
		_, self.DS_SS_tsu[self.fast, self.retrofit, :, self.r] = DCH.DS_eval_1d(self.IM_SS_d[self.r], Capacity_SS_tsu, 999)

		self.rep_time_SS_tsu[self.fast, self.retrofit, :, self.r] = np.array([self.rep_time_SS_gen_tsu[self.fast, self.retrofit, int(i), int(obj)-1] for i, obj in enumerate(self.DS_SS_tsu[self.fast, self.retrofit, :, self.r])])
		self.rep_cost_SS_tsu[self.fast, self.retrofit, :, self.r] = self.substation_cost*self.damage_ratio_SS_tsu[self.DS_SS_tsu[self.fast, self.retrofit, :, self.r].astype(int)-1]
		
		self.func_SS_tsu[self.fast, self.retrofit, self.DS_SS_tsu[self.fast, self.retrofit, :, self.r]==1, self.r] = 1
		self.func_SS_tsu[self.fast, self.retrofit, self.DS_SS_tsu[self.fast, self.retrofit, :, self.r]==2, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_SS_tsu[self.fast, self.retrofit, :, self.r] == 2))) <= 0.02)
		self.func_SS_tsu[self.fast, self.retrofit, self.DS_SS_tsu[self.fast, self.retrofit, :, self.r]==3, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_SS_tsu[self.fast, self.retrofit, :, self.r] == 3))) <= 0.20)
		self.func_SS_tsu[self.fast, self.retrofit, self.DS_SS_tsu[self.fast, self.retrofit, :, self.r]==4, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_SS_tsu[self.fast, self.retrofit, :, self.r] == 4))) <= 0.60)
		self.func_SS_tsu[self.fast, self.retrofit, self.DS_SS_tsu[self.fast, self.retrofit, :, self.r]==5, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_SS_tsu[self.fast, self.retrofit, :, self.r] == 5))) <= 1.00)
		...
		_, self.DS_SS_eq[self.fast, self.retrofit, :, self.r] = DCH.DS_eval_1d(self.IM_SS[self.r], Capacity_SS_eq, 999)
		self.rep_time_SS_eq[self.fast, self.retrofit, :, self.r] = np.array([self.rep_time_SS_gen_eq[self.fast, self.retrofit, int(i), int(obj)-1] for i, obj in enumerate(self.DS_SS_eq[self.fast, self.retrofit, :, self.r])])
		self.rep_cost_SS_eq[self.fast, self.retrofit, :, self.r] = self.substation_cost*self.damage_ratio_SS_eq[self.DS_SS_eq[self.fast, self.retrofit, :, self.r].astype(int)-1]

		self.DS_SS[self.fast, self.retrofit, :, self.r] = np.amax((self.DS_SS_tsu[self.fast, self.retrofit, :, self.r], self.DS_SS_eq[self.fast, self.retrofit, :, self.r]), axis = 0)
		self.rep_time_SS[self.fast, self.retrofit, :, self.r] = np.amax((self.rep_time_SS_tsu[self.fast, self.retrofit, :, self.r], self.rep_time_SS_eq[self.fast, self.retrofit, :, self.r]), axis = 0)
		self.rep_cost_SS[self.fast, self.retrofit, :, self.r] =  np.amax((self.rep_cost_SS_tsu[self.fast, self.retrofit, :, self.r], self.rep_cost_SS_eq[self.fast, self.retrofit, :, self.r]), axis = 0)
		self.func_SS[self.fast, self.retrofit, :, self.r] = np.amin((self.func_SS_tsu[self.fast, self.retrofit, :, self.r], self.func_SS_eq[self.fast, self.retrofit, :, self.r]), axis = 0)
	...
	def reorg_gis_data(self):
		...
		csvwrt(gis_data, header, 'epn_conn_tsu_eq')

	def write_tax_lot_conn(self):

		header = ['sim_{}' .format(i) for i in range(self.n_sims)]
		header = ','.join(header)
		header = 'tlfid' + header
		for r in range(len(self.RT)):
			tax_lot_ID = self.tax_lot_info_raw[:,2]
			data_temp = self.frac_tax_lots_conn_write[r]
			out_data = np.column_stack((tax_lot_ID, data_temp))
			filename = 'epn_conn_tsu_eq_{}' .format(self.RT[r])
			csvwrt(out_data, header, filename)

	def write_pole_failure(self):
		self.pole_failure = self.pole_failure[:, self.pole_y_n==1, :]
		header = np.empty(len(self.RT) + 1, dtype = object)
		save_data = np.zeros((np.sum(self.pole_y_n), len(self.RT) + 1))
		save_data[:,0] = np.arange(1, np.sum(self.pole_y_n) + 1, 1)
		for r in range(len(self.RT)):
			save_data[:,r+1] = np.average(self.pole_failure[r, :,:], axis = 1)
			header[r+1] = 'tsu_eq_' + str(self.RT[r])
		header[0] = 'ID'
		header = list(header)
		header = ', '.join(header)
		csvwrt(save_data, header, 'pole_failure_tsu_eq')
...
edte = EPN_damage_tsu_eq()
if edte.__dict__['write_tf'] == True:
	h5wrt(edte)
