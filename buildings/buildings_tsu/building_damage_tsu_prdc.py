...
class building_damage_tsu():
	...
	def run(self):
		...
		for self.retrofit in range(self.retrofit_vals):
			...
			for self.r in range(len(self.RT)):
				...
				self.Tsu_DamageState[self.retrofit, self.r] = self.Data[:,32:36]
	...
	def preallocate_space(self):
		...
		self.Tsu_DamageState = np.zeros(self.retrofit_vals*RT_size*len(self.Data)*4).reshape(self.retrofit_vals, RT_size, len(self.Data), 4)
	...
	def load_RTIMs(self):
		filestr = 'IM_RTP{}YR_drs.mat' .format(self.RT[self.r])
		self.IM_RTP = DCH.readmat(filestr, 'IM_RTP', dtype = 'array')
		self.Mflux = self.IM_RTP*(39.37/12)**3 	# converting mfluxfrom m3/s2 to ft3/s2
	...
	def Data_1(self, val):
			...
			self.Data[self.i, 15] = self.Mflux[self.i]
	...
	def define_building_reptime(self):
		self.building_rep_time_mu = np.array([0.5*120, 0.5*120, 360, 720] )	# restoration time % table 15.10 and 15.11
		...
		self.building_rep_time_mu2 = np.array([0.5*120, 0.5*120, 360, 720])*0.5	# restoration time % table 15.10 and 15.11
	...
	def building_eval(self):
		DS = 5-np.sum(np.heaviside(np.random.uniform(low=0, high=1, size=(self.n_lots,1)) - self.Tsu_DamageState[self.retrofit, self.r], 0.5), axis = 1) 	# damage state
...
bdt = building_damage_tsu()

if bdt.__dict__['write_h5'] == True:
	h5wrt(bdt)
