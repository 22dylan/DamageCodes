...
class water_damage_tsu_eq():
		...
		for self.fast in range(self.fast_vals):
			for self.retrofit in range(self.retrofit_vals):
				for self.r in range(len(self.RT)):
					self.define_wtp_rep_tsu_variables()
					self.define_wtp_frag_tsu_variables()
					self.wtp_eval_tsu()

					self.define_wtp_rep_eq_variables()
					self.define_wtp_frag_eq_variables()
					self.wtp_eval_eq()
					self.wtp_combine_damages()

		# ~~~ damage to water pumpings ~~~
		self.define_wps_damage_ratio()
		for self.fast in range(self.fast_vals):
			for self.retrofit in range(self.retrofit_vals):
				for self.r in range(len(self.RT)):
					for self.np in range(self.n_wps):
						self.define_wps_rep_tsu_variables()
						self.define_wps_frag_tsu_variables()
						self.wps_eval_tsu()

						self.define_wps_rep_eq_variables()
						self.define_wps_frag_eq_variables()
						self.wps_eval_eq()
	...
	def load_mat_files(self):
		...
		self.link_IM_EQ = DCH.readmat('link_IM_EQ.mat', 'link_IMs', dtype = 'array')
		self.link_IM_tsunami = DCH.readmat('link_IM_tsunami.mat', 'link_IMs', dtype = 'array')
		self.ps_IM = DCH.readmat('link_IM_tsunami.mat', 'ps_IM', dtype = 'array')
		self.wtp_IM_tsu = DCH.readmat('link_IM_tsunami.mat', 'wtp_IM', dtype = 'array')
	...
	def initialize_variables(self):
		...
		self.n_wps = 3									# number of water pumps
		...
		frag_mod_fac1 and 2 modify the meadian fragilites to account for flow rates and debris; first row is without large debris and the second one is with considering large debris
			...
			lf - low flow speed (less than 1.0 m/s)
			mf - moderate flow speed (1.0 to 5.0 m/s)
			hf - high flow speed (greater than 5. 0m/s)
				[[lf, mf, hf]
				[lf, mf, hf]]
		"""
		...
		self.outfilename = 'water_damage_tsu_eq'
	...
	def preallocate_space(self):
		...
		self.rep_time_wtp_gen_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*5).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, 5)	# note: drs, "5" may be an issue. look into generalizing
		self.DS_wtp_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.DS_wtp_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.rep_time_wtp_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.rep_time_wtp_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.rep_time_wtp = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)	# note: drs, "5" may be an issue. look into generalizing
		self.rep_time_wtp_gen_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*5).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, 5)	# note: drs, "5" may be an issue. look into generalizing
		
		self.rep_cost_wtp_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.rep_cost_wtp_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.func_wtp_tsu = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.func_wtp_eq = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.func_wtp = np.zeros(self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)

		# water pumps
		self.rep_time_wps_gen_tsu = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*5).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, 5)	# note: drs, "5" may be an issue. look into generalizing
		self.DS_wps_tsu = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.DS_wps_eq = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.rep_time_wps_tsu = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.rep_time_wps_eq = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.rep_time_wps_gen_eq = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*5).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, 5)	# note: drs, "5" may be an issue. look into generalizing

		self.rep_cost_wps_tsu = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		self.rep_cost_wps_eq = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
		...
		self.func_wps_tsu = np.zeros(self.n_wps*self.fast_vals*self.retrofit_vals*self.n_sims*RT_size).reshape(self.n_wps, self.fast_vals, self.retrofit_vals, self.n_sims, RT_size)
	...
	def define_IMs(self):
		self.IM_wpipe_sa1s = self.link_IM_EQ[:,self.r+1] # first column is link ID - units: g
		...
		self.IM_wpipe_d = self.link_IM_tsunami[:, self.r+1+len(self.RT)] # first column is link ID; flow depth - units: m
		self.IM_wpipe_d[self.IM_wpipe_d<0] = 0
		self.IM_wpipe_v = self.link_IM_tsunami[:, self.r+1]	# first column is link ID; flow speed - units: m/s
	...
	def define_wp_unit_DS(self):
		...
		_, self.wp_units_DS_tsu = DCH.DS_eval(wp_units_demand, wp_units_capacity, 999)
	...
	def define_leak_break(self):
		...
		temp1 = np.where((self.wp_units_DS_tsu*self.wp_unit_ductile) == 2)[0]
		temp2 = np.where((self.wp_units_DS_tsu*self.wp_unit_ductile) == 3)[0]
		temp3 = np.where((self.wp_units_DS_tsu*self.wp_unit_ductile) == 4)[0]

		ductile_pipe_units_leak_eq = np.random.permutation(np.sum(self.n_pipe_unit*self.ductile).astype(int))[:self.n_repairs_ductile_leak.astype(int)]
		ductile_pipe_units_break_eq = np.random.permutation(np.sum(self.n_pipe_unit*self.ductile).astype(int))[:self.n_repairs_ductile_break.astype(int)]
		
		ductile_pipe_units_leak = np.unique(np.concatenate((ductile_pipe_units_leak_eq, temp1[np.random.permutation(np.rint(len(temp1)*.02).astype(int))], temp2[np.random.permutation(np.rint(len(temp2)*.2).astype(int))], temp3[np.random.permutation(np.rint(len(temp3)*.6).astype(int))])))
		ductile_pipe_units_break = np.unique(np.concatenate((ductile_pipe_units_break_eq, np.where((self.wp_units_DS_tsu*self.wp_unit_ductile) > 4)[0])))

		temp1 = np.where((self.wp_units_DS_tsu*self.wp_unit_brittle) == 2)[0]
		temp2 = np.where((self.wp_units_DS_tsu*self.wp_unit_brittle) == 3)[0]
		temp3 = np.where((self.wp_units_DS_tsu*self.wp_unit_brittle) == 4)[0]
		
		brittle_pipe_units_leak_eq = np.random.permutation(np.sum(self.n_pipe_unit*self.brittle).astype(int))[:self.n_repairs_brittle_leak.astype(int)]
		brittle_pipe_units_break_eq = np.random.permutation(np.sum(self.n_pipe_unit*self.brittle).astype(int))[:self.n_repairs_brittle_break.astype(int)]
		brittle_pipe_units_leak = np.unique(np.concatenate((brittle_pipe_units_leak_eq, temp1[np.random.permutation(np.rint(len(temp1)*.02).astype(int))], temp2[np.random.permutation(np.rint(len(temp2)*.2).astype(int))], temp3[np.random.permutation(np.rint(len(temp3)*.6).astype(int))])))
		brittle_pipe_units_break = np.unique(np.concatenate((brittle_pipe_units_break_eq, np.where((self.wp_units_DS_tsu*self.wp_unit_brittle) > 4)[0])))
	...
	def define_wtp_damage_ratio(self):
		self.IM_wtp = np.array([0.0938, 0.3727, 0.4532, 0.6090, 0.7212, 0.9038, 1.0071, 1.1778]) # for EQ
		self.damage_ratio_wtp_tsu = np.array([0, 0.02, 0.1, 0.5, 1.0])
		self.damage_ratio_wtp_eq = np.array([0, 0.08, 0.4, 0.8, 1.0])

	def define_wtp_rep_tsu_variables(self):
		if self.fast == 0:
	...
	def define_wtp_frag_tsu_variables(self):
		n_RTP = len(self.RT)
		# note: the wtp is outside the inundation zone. 
		if self.retrofit <= 1:
			self.IM_wtp_d = 0.0*self.wtp_IM_tsu[:,self.r + n_RTP]		# first column is link ID; flow depth - units: m
		else:
			self.IM_wtp_d = 0.0*self.wtp_IM_tsu[:,self.r + n_RTP] - (5.0/3.2808) # first column is link ID; flow depth - units: m

		self.IM_wtp_d[self.IM_wtp_d<0] = 0
		IM_wtp_v = self.wtp_IM_tsu[:,self.r] # first column is link ID; flow speed - units: m/s
	...
	def wtp_eval_tsu(self):
		Capacity_wtp, self.rep_time_wtp_all = DCH.capacity_reptime_gen(self.wtp_frag_med, self.wtp_frag_covm, self.wtp_rep_time_log_med, self.wtp_rep_time_covm, self.n_sims)	# generating correlated capacity estimates for wtp
		self.rep_time_wtp_gen_tsu[self.fast, self.retrofit] = self.rep_time_wtp_all[:]
		_, self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r] = DCH.DS_eval_1d(self.IM_wtp_d, Capacity_wtp, 999)

		self.func_wtp_tsu[self.fast, self.retrofit, self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r]==1, self.r] = 1
		self.func_wtp_tsu[self.fast, self.retrofit, self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r]==2, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r] == 2))) <= 0.02)
		self.func_wtp_tsu[self.fast, self.retrofit, self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r]==3, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r] == 3))) <= 0.20)
		self.func_wtp_tsu[self.fast, self.retrofit, self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r]==4, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r] == 4))) <= 0.60)
		self.func_wtp_tsu[self.fast, self.retrofit, self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r]==5, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r] == 5))) <= 1.00)
		
		self.rep_time_wtp_tsu[self.fast, self.retrofit,:, self.r] = np.array([self.rep_time_wtp_gen_tsu[self.fast, self.retrofit, int(i), int(obj)-1] for i, obj in enumerate(self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r])])
		self.rep_cost_wtp_tsu[self.fast, self.retrofit, :, self.r] = self.damage_ratio_wtp_tsu[self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r].astype(int)-1]*30000

	def define_wtp_rep_eq_variables(self):
		if self.fast == 0:
	...
	def define_wtp_frag_eq_variables(self):
		if (self.retrofit == 0) or (self.retrofit == 2):
	...
	def wtp_eval_eq(self):
		Capacity_wtp, self.rep_time_wtp_all = DCH.capacity_reptime_gen(self.wtp_frag_med, self.wtp_frag_covm, self.wtp_rep_time_log_med, self.wtp_rep_time_covm, self.n_sims)
		self.rep_time_wtp_gen_eq[self.fast, self.retrofit] = self.rep_time_wtp_all[:]

		_, self.DS_wtp_eq[self.fast, self.retrofit, :, self.r] = DCH.DS_eval_1d(self.IM_wtp[self.r], Capacity_wtp, 999)
		self.rep_time_wtp_eq[self.fast, self.retrofit, :, self.r] = np.array([self.rep_time_wtp_gen_eq[self.fast, self.retrofit, int(i), int(obj)-1] for i, obj in enumerate(self.DS_wtp_eq[self.fast, self.retrofit, :, self.r])])
		self.rep_cost_wtp_eq[self.fast, self.retrofit, :, self.r] = self.damage_ratio_wtp_eq[self.DS_wtp_eq[self.fast, self.retrofit, :, self.r].astype(int)-1]*30000

	def wtp_combine_damages(self):
		self.DS_wtp[self.fast, self.retrofit, :, self.r] = np.amax((self.DS_wtp_tsu[self.fast, self.retrofit, :, self.r], self.DS_wtp_eq[self.fast, self.retrofit, :, self.r]), axis = 0)
		self.rep_time_wtp[self.fast, self.retrofit, :, self.r] = np.amax((self.rep_time_wtp_tsu[self.fast, self.retrofit, :, self.r], self.rep_time_wtp_eq[self.fast, self.retrofit, :, self.r]), axis = 0)
		self.rep_cost_wtp[self.fast, self.retrofit, :, self.r] =  np.amax((self.rep_cost_wtp_tsu[self.fast, self.retrofit, :, self.r], self.rep_cost_wtp_eq[self.fast, self.retrofit, :, self.r]), axis = 0)
		self.func_wtp[self.fast, self.retrofit, :, self.r] = np.amin((self.func_wtp_tsu[self.fast, self.retrofit, :, self.r], self.func_wtp_eq[self.fast, self.retrofit, :, self.r]), axis = 0)

	# ~~~~~~~~~~~~~~~~~~
	def define_wps_damage_ratio(self):
		self.IM_wps = np.array([0.0938, 0.3727, 0.4532, 0.6090, 0.7212, 0.9038, 1.0071, 1.1778]) # for EQ
		self.damage_ratio_wps_tsu = np.array([0, 0.02, 0.1, 0.5, 1.0])
		self.damage_ratio_wps_eq = np.array([0, 0.05, 0.38, 0.8, 1.0])

	def define_wps_rep_tsu_variables(self):
		if self.fast == 0:
			...
			self.wps_rep_time_mu = np.array([1, 6, 20, 90])*0.5 # mean repair time for water treatement plants for DS2-DS5
			self.wps_rep_time_std = np.array([1, 6, 20, 90])*0.5 # std for repair time	
	...
	def define_wps_frag_tsu_variables(self):
		n_RTP = len(self.RT)
		if self.retrofit <= 1:
	...
	def wps_eval_tsu(self):
		Capacity_wps, self.rep_time_wps_all = DCH.capacity_reptime_gen(self.wps_frag_med, self.wps_frag_covm, self.wps_rep_time_log_med, self.wps_rep_time_covm, self.n_sims)	# generating correlated capacity estimates for wps
		self.rep_time_wps_gen_tsu[self.np, self.fast, self.retrofit] = self.rep_time_wps_all[:]
		_, self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r] = DCH.DS_eval_1d(self.IM_wps_d, Capacity_wps, 999)

		self.func_wps_tsu[self.np, self.fast, self.retrofit, self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r]==1, self.r] = 1
		self.func_wps_tsu[self.np, self.fast, self.retrofit, self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r]==2, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r] == 2))) <= 0.02)
		self.func_wps_tsu[self.np, self.fast, self.retrofit, self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r]==3, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r] == 3))) <= 0.20)
		self.func_wps_tsu[self.np, self.fast, self.retrofit, self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r]==4, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r] == 4))) <= 0.60)
		self.func_wps_tsu[self.np, self.fast, self.retrofit, self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r]==5, self.r] = np.logical_not(np.random.uniform(0,1,(np.sum(self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r] == 5))) <= 1.00)
		
		self.rep_time_wps_tsu[self.np, self.fast, self.retrofit,:, self.r] = np.array([self.rep_time_wps_gen_tsu[self.np, self.fast, self.retrofit, int(i), int(obj)-1] for i, obj in enumerate(self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r])])
		self.rep_cost_wps_tsu[self.np, self.fast, self.retrofit, :, self.r] = self.damage_ratio_wps_tsu[self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r].astype(int)-1]*150

	def define_wps_rep_eq_variables(self):
		if self.fast == 0:
			...
			# note: drs, should these be multiplied by 1/2?
			self.wps_rep_time_mu = np.array([0.9, 3.1, 13.5, 35]) # mean repair time for water treatement plants for DS2-DS5
			self.wps_rep_time_std = np.array([0.3, 2.7, 10, 18]) # std for repair time
	...
	def define_wps_frag_eq_variables(self):
		if (self.retrofit == 0) or (self.retrofit == 2):
			self.wps_frag_med = np.array([0.13, 0.28, 0.66, 1.5]) # mean repair time for water treatement plants for DS2-DS5
			self.wps_frag_beta = np.array([0.6, 0.5, 0.65, 0.8]) # std for repair time		
		else:
			self.wps_frag_med = np.array([0.15, 0.36, 0.66, 1.5]) # mean repair time for water treatement plants for DS2-DS5
			self.wps_frag_beta = np.array([0.7, 0.65, 0.65, 0.8]) # std for repair time

		self.wps_frag_covm = self.wps_frag_beta[:,None]*self.wps_frag_beta

	def wps_eval_eq(self):
		Capacity_wps, self.rep_time_wps_all = DCH.capacity_reptime_gen(self.wps_frag_med, self.wps_frag_covm, self.wps_rep_time_log_med, self.wps_rep_time_covm, self.n_sims)
		self.rep_time_wps_gen_eq[self.np, self.fast, self.retrofit] = self.rep_time_wps_all[:]

		_, self.DS_wps_eq[self.np, self.fast, self.retrofit, :, self.r] = DCH.DS_eval_1d(self.IM_wps[self.r], Capacity_wps, 999)
		self.rep_time_wps_eq[self.np, self.fast, self.retrofit, :, self.r] = np.array([self.rep_time_wps_gen_eq[self.np, self.fast, self.retrofit, int(i), int(obj)-1] for i, obj in enumerate(self.DS_wps_eq[self.np, self.fast, self.retrofit, :, self.r])])
		self.rep_cost_wps_eq[self.np, self.fast, self.retrofit, :, self.r] = self.damage_ratio_wps_eq[self.DS_wps_eq[self.np, self.fast, self.retrofit, :, self.r].astype(int)-1]*150

	def wps_combine_damages(self):
		self.DS_wps[self.np, self.fast, self.retrofit, :, self.r] = np.amax((self.DS_wps_tsu[self.np, self.fast, self.retrofit, :, self.r], self.DS_wps_eq[self.np, self.fast, self.retrofit, :, self.r]), axis = 0)
	...
	def write_link_failure(self):
		...
		header = list(header)
		header = ', '.join(header)
		csvwrt(save_data, header, 'link_failure_tsu_eq')
...
wdte = water_damage_tsu_eq()
if wdte.__dict__['write_tf'] == True:
	h5wrt(wdte)
